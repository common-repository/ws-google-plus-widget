<?php
/*
 Plugin Name: WS Google Plus Widget  
 Plugin URI: https://wordpress.org/plugins/ws-google-plus-widget/
 Description: Our WS Google Plus Widget plugin will help you to display google plus widget on your wordpress website, just add google plus badge widget to your sidebar and use it. 
 Version: 1.0       
 Author: Web Shouter       
...
class ws_google_plus_badge extends WP_Widget {
	                                  
	function __construct() {                                                   
		parent::__construct(              
			'ws_google_plus_widget', //Base ID                     
			__( 'WS Google Plus Widget', 'ws-google-plus-widget' ), //Name
			array( 'description' => __( 'Display google plus widget!', 'ws-google-plus-widget' ), ) //Args
		);
	}      
	                             
	public function form( $instance ) {
   
		$defaults = array( 'title' => __('Find us on Google Plus', 'webshouter'), 'page_type' => 'profile', 'page_url' => '', 'color_scheme' => 'light', 'layout_type' => 'portrait', 'widget_width' => '300', 'show_cover_photo' => 'on', 'show_tagline' => 'on' );
		...
		<p>
			<label for="<?php echo $this->get_field_id('page_type'); ?>"><?php _e('Page type','webshouter'); ?>:</label> 
        ...
        <p>
            <label
                for="<?php echo $this->get_field_id( 'page_url' ); ?>"><?php _e( 'Google+ Page URL','webshouter'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('color_scheme'); ?>"><?php _e('Color Scheme','webshouter'); ?>:</label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id('layout_type'); ?>"><?php _e('Layout Type','webshouter'); ?>:</label> 
         ...
         <p>
            <label for="<?php echo $this->get_field_id( 'widget_width' ); ?>"><?php _e( 'Width', 'webshouter' ); ?>:</label>
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id( 'show_cover_photo' ); ?>"><?php _e( 'Show Cover Photo', 'webshouter' ); ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo $this->get_field_id( 'show_tagline' ); ?>"><?php _e( 'Show Tagline', 'webshouter' ); ?></label>
        </p>
    <?php
    }
...
// register widget
function register_ws_google_plus_badge_widget() {
    register_widget( 'ws_google_plus_badge' );
}
add_action( 'widgets_init', 'register_ws_google_plus_badge_widget' );
